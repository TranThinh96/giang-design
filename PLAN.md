...
3. **Products and portfolio describe two different businesses.** The 8 services are a TP.HCM offset print shop (bao bì, tem nhãn, catalogue). The 9 portfolio pieces are signage, exhibition booths and Tết decoration in Nha Trang. Confirm which is the actual scope — if the company does both, the portfolio needs print categories added; if it's print only, the portfolio needs replacing.
4. **Stats need verification.** `15+ năm`, `1.200+ đơn hàng/năm`, `48h mẫu in thử`, `ΔE ≤ 3`, and the six machine capacities are all claims a customer may hold them to.
